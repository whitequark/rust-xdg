...
impl BaseDirectoriesError {
    fn new(kind: BaseDirectoriesErrorKind) -> BaseDirectoriesError {
        BaseDirectoriesError { kind: kind }
    }
}
...
impl UserDirectories {
    fn new() -> Result<UserDirectories, BaseDirectoriesError> {
    ...
    fn with_basedir(basedir: BaseDirectories) -> Result<UserDirectories, BaseDirectoriesError> {
        let home = dirs::home_dir().ok_or(Error::new(HomeMissing))?;

        let user_dirs = basedir.config_home.join("user-dirs.dirs");
        let f = File::open(user_dirs).expect("");
        let mut reader = BufReader::new(f);

        let mut str = String::new();
        reader.read_to_string(&mut str);

        let env = dotenv_parser::parse_dotenv(&str).expect("");
...
fn get_userpath(env: &BTreeMap<String, String>, name: &str, home: &PathBuf) -> Option<PathBuf> {
...
#[cfg(test)]
fn make_absolute<P>(path: P) -> PathBuf
where
    P: AsRef<Path>,
{
...
#[cfg(test)]
fn make_relative<P>(path: P) -> PathBuf
where
    P: AsRef<Path>,
{
    iter_after(
        path.as_ref().components(),
        env::current_dir().unwrap().components(),
    )
    .unwrap()
    .as_path()
    .to_owned()
...
fn test_runtime_good() {
    ...
    let xd = BaseDirectories::with_env(
        ...
        &*make_env(vec![
            ...
            (
                "XDG_RUNTIME_DIR",
                format!("{}/test_files/runtime-good", cwd),
            ),
        ]),
    )
    .unwrap();
...
fn test_get_file() {
    ...
    let xd = BaseDirectories::with_env(
        ...
        &*make_env(vec![
            ...
            (
                "XDG_RUNTIME_DIR",
                format!("{}/test_files/user/runtime", cwd),
            ),
        ]),
    )
    .unwrap();
    ...
    assert_eq!(
        file,
        PathBuf::from(&format!("{}/test_files/user/data/user_data.file", cwd))
    );

    let file = xd.get_cache_file("user_cache.file");
    assert_eq!(
        file,
        PathBuf::from(&format!("{}/test_files/user/cache/user_cache.file", cwd))
    );
