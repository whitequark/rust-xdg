...
impl XdgDirs
{
    ...
    fn new_with_env<F: Fn(&str) -> Option<Vec<u8>>>(env: F) -> XdgDirs {
        ...
        let data_home = getenv_one(|n: &str| env(n), "XDG_DATA_HOME")
                            .unwrap_or(home.join(".local/share"));
        let config_home = getenv_one(|n: &str| env(n), "XDG_CONFIG_HOME")
                              .unwrap_or(home.join(".config"));
        let cache_home = getenv_one(|n: &str| env(n), "XDG_CACHE_HOME")
                             .unwrap_or(home.join(".cache"));
        let data_dirs = getenv_many(|n: &str| env(n), "XDG_DATA_DIRS")
                            .unwrap_or(vec![Path::new("/usr/local/share"),
                                            Path::new("/usr/share")]);
        let config_dirs = getenv_many(|n: &str| env(n), "XDG_CONFIG_DIRS")
                              .unwrap_or(vec![Path::new("/etc/xdg")]);
...
fn test_files_exists() {
    ...
    assert!(Path::new("test_files/runtime-bad")
                .stat().unwrap().perm.intersects(io::GROUP_RWX | io::OTHER_RWX));
}
...
fn test_runtime_good() {
    ...
    let mut root: Vec<String> = root.into_iter()
                                    .map(|p| make_relative(&p).as_str().unwrap().to_string())
                                    .collect();
    root.sort();
    assert_eq!(root,
               vec!["test_files/runtime-good/bar".to_string(),
                    "test_files/runtime-good/foo".to_string()]);
...
fn test_lists() {
    ...
    let mut files = files.into_iter().map(|p| make_relative(&p).as_str().unwrap().to_string())
                                     .collect();
    files.sort();
    let files = files;
    assert_eq!(files,
